...
║                                                                    ║
║         SENATUS · LEGIONES · PROVINCIAE · MARE · AURUM · VOX      ║
╚════════════════════════════════════════════════════════════════════╝
</pre>

# 🏛 ROMA AETERNA

### **IMPERIUM VIVUM · VOX ROMANA**

*Exercitus · Mare Nostrum · Civitates Artificiales · Bellum Universale*

**Большая историческая 4X-стратегия о Риме — в терминале, на Python, с полным пакетом русской озвучки игровых цитат.**

[![Version](https://img.shields.io/badge/version-3.5.2%20Stability%20Fix-B22222)](#-текущая-версия)
...
[![Voiceover](https://img.shields.io/badge/voiceover-Russian%20quotes%20%7C%20Silero-7B2CBF)](#-русская-озвучка--vox-romana)
...
[![CI](https://github.com/Aqua666-prog/Roma-Aeterna/actions/workflows/python-app.yml/badge.svg)](https://github.com/Aqua666-prog/Roma-Aeterna/actions/workflows/python-app.yml)
...
[![Last Commit](https://img.shields.io/github/last-commit/Aqua666-prog/Roma-Aeterna?logo=github)](https://github.com/Aqua666-prog/Roma-Aeterna/commits/main)
[![Repo Size](https://img.shields.io/github/repo-size/Aqua666-prog/Roma-Aeterna)](https://github.com/Aqua666-prog/Roma-Aeterna)
...
&nbsp;·&nbsp;
[**💻 GitHub**](https://github.com/Aqua666-prog/Roma-Aeterna)
&nbsp;·&nbsp;
[**📦 Быстрый старт**](#-быстрый-старт)
&nbsp;·&nbsp;
[**🎙 Озвучка**](#-русская-озвучка--vox-romana)
&nbsp;·&nbsp;
[**🗺 ROADMAP**](#-roadmap)
&nbsp;·&nbsp;
...
| 🏛 Государство | ⚔ Живая война | 🎙 Vox Romana | 📱 Доступность |
|:---:|:---:|:---:|:---:|
| Сенат, экономика, города и провинции | автономные армии, флоты и кампании | технологии, великие люди и чудеса света | Windows, Linux, Android / Termux |

> [!NOTE]
> Готовые WAV уже включены в репозиторий. При отсутствии аудиоплеера или отдельного файла игра продолжает работу без падения — просто без звука.
...
- [Русская озвучка](#-русская-озвучка--vox-romana)
...
- [Как выглядит игра](#-как-выглядит-игра)
...
Этот README актуализирован для **Roma Aeterna v3.5.2 — Stability Fix · Vox Romana**. Релиз сохраняет масштабную государственную симуляцию ветки 3.x и добавляет готовую русскую озвучку цитат технологий, великих людей и чудес света.

| Компонент | Состояние в релизе | Назначение |
|---|---:|---|
| Игровое ядро | `3.5.2` | основной цикл, Рим, меню, сохранения, миграции и проверки целостности |
| Vox Romana | `integrated` | безопасное неблокирующее воспроизведение готовых WAV через `roma_voice.py` |
| Russian Voice Pack | `included` | аудио технологий, великих людей и чудес в каталоге `audio/` |
| Silero Generator | `included` | повторная генерация WAV, словарь ударений и произношение латинских выражений |
| Exercitus Romanus | `3.x` | оперативные армии из легионов, ауксилий, артиллерии и эскадр |
| Mare Nostrum | `3.x` | отдельное состояние флота, порты, содержание, блокада и морская война |
| Civitates Artificiales | `3.x` | экономика, исследования, религия, армия и флот иностранных держав |
| Chronica Urbis | `active` | анналы, технический журнал, причины и последствия событий |

### Что нового в v3.5.2

1. **Добавлен полный пакет русской озвучки игровых цитат.** Озвучиваются изученные технологии, появившиеся великие люди и построенные чудеса света.
2. **Воспроизведение не блокирует игру.** Модуль `roma_voice.py` запускает системный плеер в фоне и мягко пропускает звук, если WAV или подходящий аудиобэкенд отсутствуют.
3. **Озвучка управляется из `roma_settings.json`.** Можно отключить её, изменить громкость и выбрать, должна ли новая реплика прерывать предыдущую.
4. **Готовые WAV уже находятся в репозитории.** Для обычной игры Silero, Torch и генератор устанавливать не требуется.
5. **Генератор оставлен для разработчиков.** `data/quotes/generate_voiceover_silero.py` поддерживает ручные ударения, словарь произношения латинских выражений, предварительный просмотр и перезапись аудио.
6. **Сохранён безопасный fallback.** Проблема со звуком не должна приводить к падению партии или повреждению сохранения.
7. **Системы ветки 3.x остаются совместимыми.** Оперативные армии, автономные державы, море, экономика, династии и Consilium Orbis продолжают работать в общей архитектуре.
...
flowchart LR
    ...
    F --> G["v3.5.2<br/>Stability Fix · Vox Romana"]
```
...
| **v3.5.2 — Stability Fix · Vox Romana** | интегрированы готовые WAV, неблокирующий аудиомодуль, настройки громкости и генератор Silero с вручную проверенными ударениями |
...
| **Озвучка** | готовые русские WAV для цитат технологий, великих людей и чудес света |
...
| 🎙 Звук | Vox Romana | готовые WAV, безопасный системный плеер, громкость, прерывание и тихий fallback |
...
## 🎙 Русская озвучка — *Vox Romana*

В версии 3.5.2 в проект встроен готовый пакет **синтезированной русской озвучки**. Реплики созданы через Silero TTS, а ударения, редкие имена, античные топонимы и латинские выражения дополнительно проверялись и корректировались вручную.

### Что озвучивается

- завершение исследования технологии;
- появление великого человека;
- завершение строительства чуда света.

Готовые файлы лежат в трёх каталогах:
...
`roma_voice.py` ищет WAV по категории и идентификатору события, выбирает доступный системный аудиобэкенд и запускает звук неблокирующе. Поддерживаются:
...
> [!IMPORTANT]
> Озвучка является необязательным слоем. Если файл повреждён, отсутствует или в системе нет плеера, игра продолжает работу без звука.

### Настройки

Параметры находятся в `roma_settings.json`:
...
- `voiceover_enabled` — включает или отключает озвучку;
- `voiceover_volume` — громкость от `0` до `100` для поддерживающих её аудиобэкендов;
- `voiceover_interrupt_previous` — новая реплика останавливает предыдущую.

### Повторная генерация WAV

Для обычной игры этот шаг **не нужен**: готовая озвучка уже включена в репозиторий. Он предназначен для разработчиков, которые изменили цитаты, ударения или голос.

Из корня проекта:

```bash
python -m pip install torch omegaconf
python data/quotes/generate_voiceover_silero.py --preview 3
python data/quotes/generate_voiceover_silero.py --overwrite
```

На Termux может потребоваться:

```bash
pip install torch omegaconf --break-system-packages
```

Дополнительный автоматический акцентор `silero-stress` необязателен: ручной `stress_dictionary.json` и встроенная постановка ударений Silero остаются доступными без него.
...
Для озвучки рекомендуется `mpv`; в приведённой выше команде он уже установлен.

---

## 🎧 Воспроизведение озвучки

Готовые WAV загружаются вместе с проектом. Дополнительная настройка зависит только от платформы:

| Платформа | Рекомендуемый аудиобэкенд | Установка |
|---|---|---|
| Windows | встроенный `winsound` | не требуется |
| Termux | `mpv` | `pkg install mpv` |
| Linux | `mpv`, `ffplay`, `paplay` или `aplay` | через пакетный менеджер системы |
| macOS | встроенный `afplay` | не требуется |

При отсутствии аудиобэкенда игра запускается нормально и просто пропускает аудио.
...
- `mpv` или другой поддерживаемый системный плеер для озвучки на Linux/Termux.

### Обязательные зависимости

**Нет для игрового ядра.** Игра сохраняет работоспособность на стандартной библиотеке Python. Rich, Textual, Pillow, Lupa и системный аудиоплеер являются опциональными: без них включается упрощённый интерфейс, встроенные данные или беззвучный режим.
...
```text
Roma-Aeterna/
├── roma_aeterna.py                  # ядро v3.5.2, цикл хода, меню и сохранения
├── roma_voice.py                    # безопасное неблокирующее воспроизведение WAV
...
├── audio/                            # готовый русский voice pack
│   ├── tech/*.wav
│   ├── great_people/*.wav
│   └── world_wonders/*.wav
├── data/quotes/
...
├── VOICEOVER_INSTALL.txt            # краткая инструкция по аудио
├── requirements-optional.txt        # необязательные визуальные зависимости
├── roma_settings.json               # настройки игры и озвучки
...
flowchart LR
    ...
    MAIN --> VOICE[roma_voice.py]
    VOICE --> AUDIO[audio / готовые WAV]
...
1. Убедитесь, что WAV действительно находятся в `audio/tech`, `audio/great_people` и `audio/world_wonders`.
2. В Termux установите плеер:

```bash
pkg install mpv
```

3. Проверьте параметры `voiceover_enabled` и `voiceover_volume` в `roma_settings.json`.
4. Помните, что отсутствие звука не считается критической ошибкой: игра намеренно продолжает работу в беззвучном режиме.
...
- сообщения о неверных ударениях, произношении имён и латинских выражений;
...
- [x] полный пакет русской озвучки цитат технологий, великих людей и чудес света;
- [x] безопасный неблокирующий аудиомодуль и генератор Silero со словарём ударений.
...
Legiones vigilant.
Classis navigat.
Vox historiae resonat.
Fortuna tacet.
```

**ROMA AETERNA · v3.5.2 · STABILITY FIX · VOX ROMANA**
